...
do
    ...
    1)
  			...
  			echo ---------------------------------------------------------------
  			read -p "Digite el nombre del Common Name del Certificado   : " NameDNS
  			echo ---------------------------------------------------------------
			...
			openssl req -key ~/certificates/$NameDNS/$NameDNS.key -out ~/certificates/$NameDNS/$NameDNS.csr -subj \
  			/C=$vCountry/ST=$vState/L=$vLocality/O=$vOrganization/OU=$vUnitOrganization/CN=$NameDNS/emailAddress=$vemail -new -sha256
		...
		6)
			...
			case $choice in
					...
					C)
						...
						cd ~/certificates/$NameDNS
           				keytool -importkeystore -srckeystore ~/certificates/$NameDNS/$NameDNS.pfx -srcstoretype pkcs12 -srcalias 1 -destkeystore ~/certificates/$NameDNS/$NameDNS.jks -deststoretype jks -deststorepass Banco123* -destalias $NameDNS
